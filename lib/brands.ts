// Brand marks for the tech stack and org logos. Path data comes from the
// simple-icons package; OpenAI and AWS are not in simple-icons, so their paths
// come from @lobehub/icons-static-svg (both 24x24 viewBoxes).
import {
  siAnthropic,
  siCss,
  siDocker,
...
export type Brand = { title: string; path: string; hex: string };
...
export const brands: Record<string, Brand> = {
  ...
  LeetCode: siLeetcode,
  Paytm: siPaytm,
  "JPMorgan Chase": siChase,
};

// Aliases so content strings written different ways resolve to one mark.
...
export function brandFor(name: string): Brand | undefined {
  return brands[name] ?? brands[aliases[name] ?? ""];
}

/** True when the brand color is too dark to read on a dark surface. */
export function isInkBrand(hex: string): boolean {
