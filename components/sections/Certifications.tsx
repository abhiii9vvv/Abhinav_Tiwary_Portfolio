    ...
    <Section id="certifications" eyebrow="Certifications" title="Continued learning.">
      <ul className="flex flex-col gap-1">
        {certifications.map((cert, i) => {
          const color = colorForTag(cert.name);
          return (
            <Reveal key={cert.name} delay={i * 0.03}>
              <li
                className="flex flex-col justify-between gap-1 rounded-lg border-l-4 px-3 py-4 transition-colors duration-200 hover:bg-bg-raised sm:flex-row sm:items-center"
